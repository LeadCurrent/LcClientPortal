...
namespace Web
{
    public class DegreeLevelController : Controller
    {
        ...
        public async Task<DegreeLevelVM> getDegreeLevelList()
        {
            ...
            Model.DegreeLevels = await DegreeLevelDL.GetDegreeLevels(CompanyId);
            return Model;
        }
        ...
        public async Task<IActionResult> Create(DegreeLevelVM ViewModel, string Action)
        {
            try
            {
                ...
                {
                    ViewModel.DegreeLevel.CompanyId = Int32.Parse(User.Claims.Where(x => x.Type == "CompanyId").FirstOrDefault().Value);
